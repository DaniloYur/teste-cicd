...
function gerarTabelaHistoricoAtualizacao(paramCodIdeCli){
    ...
    $('#flgPensao').val(urlParametros.get("flgPensao"));

    let jsonData = consumidor.executarServico("controleCompensacao/consultarHistorico", params);
    ...
    if($('#flgPensao').val() == '1') {
        cabecalhoDaTabela     += '<th>DEPENDENTE ALTERADO</th>';
    }
    ...
    if(JSON.stringify(jsonData.listaHistoricos) === '[]'){
        ...
        $.each(jsonData.listaHistoricos, function(eachIndice, eachHistorico){
            ...
            if($('#flgPensao').val() == '1') {
                ...
                corpoDaTabela +=      '</td>';
                corpoDaTabela +=     '<td>';
                corpoDaTabela +=          eachHistorico.nomBeneficiario;
                corpoDaTabela +=      '</td>';
                corpoDaTabela +=     '<td>';
