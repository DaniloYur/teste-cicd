    ...
    $.ajax(settingsIndices).done(function (response) {
        ...
        $(keysIndices).each(function () {
           ...
           // if (responseIndices["chave"] == "NUMERO DO PROCESSO") {
			//	if(comboGrupoDeDocumentos != 1 || (comboGrupoDeDocumentos == 1 && responseIndices["valor"] != "NAO INFORMADO")){
			///		responseIndices["valor"] = protocoloComZero1;
			//	}
           // }
    ...
    if (numSeq != null || numSeq != undefined || numSeq != "") {
        $.ajax(settingsIndicesProtocolo).done(function (response1) {
            ...
            $(keysIndicesProtocolo).each(function () {
				...
				//verifica se existe indice vazio
				console.log(responseIndicesProtocolo["chave"]);
				if(responseIndicesProtocolo["chave"] != "_FILIAL" && responseIndicesProtocolo["valor"] == ''){
					existeIndiceVazio = true;
				}
            });
			...
			//verificar se deve ser executada a alteração de indices
			if(existeIndiceVazio){
				...
				alterarIndicesArquivoCarregadoSemValidar(numSeqAlterar, codBeneficioAlterar);
...
function salvarArquivo(filial, numeroCaixa, tipoDocumento) {
    ...
    let settingsSalvarArquivo = {
       	url: urlDominioSigeprev + 
       		urlContextoSigeprev + 
       		"/digitalizarDocumentoServlet/DigitalizarDocumentoServlet?acao=EA&origem=COMPREV" + 
       		"&numSeq=" + numSeq + 
       		"&assinar=false" + 
       		"&filial=" + filial + 
       		"&caixa=" + numeroCaixa + 
       		"&" + $("#inputIndiceEditavelProtocoloTipoDocumento").serialize() + 
       		"&grupoDocumento=" + comboGrupoDeDocumentos + 
       		"&codFilial=" + $("#inputIndiceEditavelProtocoloFilial").val() + 
       		"&tipoEnvio=CNPJ" +
       		"&codGrupoDocumento=" + $('#comboGrupoDeDocumentos :checked').val(),
       	method: "GET",
        headers: {
            "tokenSigeprev": getCookieSigeprev("tokenSigeprev")
        },
...
function alterarIndicesArquivoCarregado() {
    ...
    exibirAguardeBootstrap();
    let jsonData = alterarIndicesArquivoCarregadoSemValidar(numSeqAlterar, codBeneficioAlterar);
    sumirAguardeBootstrap();
...
function alterarIndicesArquivoCarregadoSemValidar(_numseqAlterar, _codBeneficioAlterar) {
	...
	params.numSeq = _numseqAlterar;
	params.codBeneficio = _codBeneficioAlterar;
