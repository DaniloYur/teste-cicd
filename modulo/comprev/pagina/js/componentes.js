...
// Dialog Jquery UI 1.12 => adaptado para old template
function alertNovo2(msgEmHtml, methodCall){
    $(function() {
        $("#dialog-confirm").attr("title", 'MENSAGEM S\u00C3O PAULO PREVID\u00CANCIA');
        $("#dialog-confirm").html(msgEmHtml);
        $("#dialog-confirm").dialog({
...
// Dialog Jquery UI 1.12 => template novo responsivo.
function alertNew2(msgEmHtml, methodCall){
    $(function() {
        if (screen.availWidth >= 1024) {
            largura = 380;
        } else if (screen.availWidth >= 768) {
        ...
        if ($("#dialog-msg",window.parent.document).length > 0) {
            ...
            $("#dialog-msg .ui-dialog-buttonset button",window.parent.document).click(function(){
                botaoDialog("#dialog-msg");
                if (typeof (methodCall) == 'function')
                    methodCall();
            });
            $("#dialog-msg .ui-dialog-titlebar .ui-dialog-titlebar-close",window.parent.document).click(function(){
                botaoDialog("#dialog-msg");
                if (typeof (methodCall) == 'function')
                    methodCall();
            });
