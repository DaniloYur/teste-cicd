...
function tirarMascaraDeValor(valor) {
	var c, ret;
	ret = "";
	for (i = 0; i < valor.length; i++) {
		c = valor.substring(i,i+1);
		if (isdigit(c))
			ret = ret + c;
	}
...
function validaCampoCpfComMascara2SemDigitosIguais(campo) {
	if (campo.value != '') {
		...
		if (!ValidaCPFSemNumerosIguais(cpf)){
			...
			erros = erros + msg + ' DIGITADO N\u00C3O \u00E9 VALIDO.\n';
			return true;
		}
	}
	return false;
...
function validaPIS2(campo)
{
	...
	{
		erros = erros + '- O campo '+campo.title+ ' � inválido.\n';
		return true;
	}	
